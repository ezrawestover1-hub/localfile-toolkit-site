...
test("product entitlement summary keeps LedgerHarbor ownership independent", () => {
...
test("product entitlement summary keeps CaptionShift ownership independent", () => {
  const captionStandard = summarizeEntitlements([{ product_key: "captionshift", plan_key: "standard", status: "active" }]);
  assert.equal(captionStandard.products.captionshift, "standard");
  assert.equal(captionStandard.products.ledgerlift, "free");
  assert.equal(captionStandard.products.calendarflow, "free");
  const captionPlus = summarizeEntitlements([{ product_key: "captionshift", plan_key: "standard", status: "active" }, { product_key: "captionshift", plan_key: "plus", status: "active" }]);
  assert.equal(captionPlus.products.captionshift, "plus");
  const otherProduct = summarizeEntitlements([{ product_key: "ledgerlift", plan_key: "plus", status: "active" }]);
  assert.equal(otherProduct.products.captionshift, "free");
  const revoked = summarizeEntitlements([{ product_key: "captionshift", plan_key: "plus", status: "revoked" }]);
  assert.equal(revoked.products.captionshift, "free");
  const bundle = summarizeEntitlements([{ product_key: "suite", plan_key: "bundle", status: "active" }]);
  assert.equal(bundle.products.captionshift, "plus");
});
...
test("health is minimal and directory routes serve their index assets", async () => { const env = dbEnv(); const health = await handleRequest(new Request("https://example.test/api/health"), env); assert.deepEqual(await health.json(), { status: "ok" }); const page = await handleRequest(new Request("https://example.test/ledgerlift/"), env); assert.equal(await page.text(), "static:/ledgerlift/index.html"); });
test("readiness reports incomplete production configuration without exposing secrets", async () => { const env = dbEnv(); const response = await handleRequest(new Request("https://example.test/api/readiness"), env); assert.equal(response.status, 503); const body = await response.json(); assert.equal(body.ready, false); assert.equal(body.checks.database, true); assert.equal(body.checks.licenseSigningSecret, true); assert.equal(body.checks.durableRateLimiter, false); assert.doesNotMatch(JSON.stringify(body), /sandbox|test_|live_|pri_/i); assert.equal(Object.hasOwn(body.checks, "PADDLE_API_KEY"), false); });
test("durable rate limiting can be required in production", async () => { const env = dbEnv(); env.REQUIRE_DURABLE_RATE_LIMITER = "true"; const response = await handleRequest(new Request("https://example.test/api/contact", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: "A", email: "buyer@example.com", topic: "General question", product: "LocalFile Toolkit", subject: "Hello", message: "A question", consent: true }) }), env); assert.equal(response.status, 429); });
