...
form.addEventListener("submit", async (event) => {
  ...
  if (response?.redirected) location.assign(response.url); else { const result = await response?.json().catch(() => ({})); if (result?.ok && result.redirect) location.assign(result.redirect); else message.textContent = result?.message || `Unable to reset the password (server status ${response?.status || "unavailable"}).`; }
});
