...
- Follow the [production rollout runbook](PRODUCTION_ROLLOUT.md) and keep `/api/readiness` green before accepting traffic.
- Configure the single shared `/checkout-portal/` with the environment-specific Paddle values supplied by Worker configuration; production uses live values while local development defaults to sandbox. The public comparison page is `/pricing.html`.
...
The official public pricing page is `/pricing.html`. Contact and refund requests use the Worker endpoints `/api/contact` and `/api/refund-request`; configure the provider described in [SUPPORT_EMAIL_SETUP.md](SUPPORT_EMAIL_SETUP.md) before relying on online delivery. Checkout is environment-specific: local development remains sandbox-safe, while production is enabled only after the external Paddle, legal, email, and operational gates are satisfied.
