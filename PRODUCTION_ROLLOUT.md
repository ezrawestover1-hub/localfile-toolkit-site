...
This runbook covers the external Sprint 2 work that cannot be safely completed by
editing the repository alone. Keep checkout disabled until every gate is verified.
...
2. Create a live webhook destination for `/api/paddle/webhook`, subscribe to
   `transaction.completed`, and set `PADDLE_WEBHOOK_SECRET` as a Worker secret.
...
4. Verify a live client-side token and all live `pri_...` IDs in
   `checkout-portal/paddle-config.js`. Keep private API keys and webhook secrets
   out of the repository.
5. Run one controlled purchase for an individual Standard plan and one for the
...
6. Confirm a duplicate webhook is idempotent, a failed D1 fulfillment is retried,
   the Paddle customer portal opens only for an authenticated owner, and a refund
   request reaches support.
7. Only after those checks pass, change `checkoutEnabled` to `true` and
   `environment` to `production` in the shared checkout config, commit, and deploy.
