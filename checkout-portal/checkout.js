(() => {
  ...
  const PRODUCTS = Object.freeze({
    ...
    suite:{name:"Complete Plus Bundle",icons:Object.values(window.PRODUCT_ICONS).map(item=>item.icon),description:"One-time Plus entitlement for all five private converter products. LedgerLift Plus controls are available now; the other product Plus controls will be added before launch.",home:"../index.html",benefits:{bundle:["LedgerLift Plus with saved profiles, advanced mapping, categorization, duplicate review, and reports","PixelPort Plus","ContactCraft Plus","CalendarFlow Plus","CaptionShift Plus","Save $26.96 versus five separate Plus licenses"]}}
  });
  ...
  Object.values(PRODUCTS).forEach(item=>{if(item.benefits.plus && !["LedgerLift","PixelPort","ContactCraft"].includes(item.name))item.benefits.plus=["Everything in Standard","Plus-specific controls — planned","Not included in the current release"];});
