(() => {
  ...
  $("sampleBtn").addEventListener("click", () => { const canvas = document.createElement("canvas"); canvas.width = 900; canvas.height = 540; const context = canvas.getContext("2d"), gradient = context.createLinearGradient(0, 0, 900, 540); gradient.addColorStop(0, "#24366f"); gradient.addColorStop(1, "#f0ad4e"); context.fillStyle = gradient; context.fillRect(0, 0, 900, 540); context.fillStyle = "#fff"; context.font = "bold 72px system-ui"; context.fillText("PixelRefinery", 250, 260); context.font = "32px system-ui"; context.fillText("Private image conversion", 260, 320); canvas.toBlob((blob) => load(new File([blob], "pixelport-sample.png", { type: "image/png" }), true), "image/png"); });
