(() => {
  ...
  function createPanel() {
    ...
    $("saveProfile").addEventListener("click", () => { const name = window.prompt("Name this LedgerLift profile"); if (!name?.trim()) return; const all = profiles(); all[name.trim()] = profileData(); saveProfiles(all); refreshProfiles(); $("ledgerliftProfile").value = name.trim(); $("plusStatus").textContent = `Saved profile “${name.trim()}”.`; });
    $("loadProfile").addEventListener("click", () => { const data = profiles()[$("ledgerliftProfile").value]; if (!data) return; Object.entries(data).forEach(([id, value]) => { const control = $(id); if (control && value !== undefined) control.value = value; }); $("plusStatus").textContent = "Profile loaded. Analyze again to apply the mapping."; });
    $("deleteProfile").addEventListener("click", () => { const select = $("ledgerliftProfile"), all = profiles(); if (!select.value) return; delete all[select.value]; saveProfiles(all); refreshProfiles(); $("plusStatus").textContent = "Profile deleted."; });
    $("applyRules").addEventListener("click", applyRules);
    $("dedupe").addEventListener("click", () => { duplicateRows(); renderPlusRows(); report(); $("plusStatus").textContent = "Matching date, description, and amount rows are marked as duplicates and excluded from IIF/report totals."; });
